...
def pre_delete_story(sender, instance, **kwargs):
    ...
    find_post = Post.objects.filter(
        author=author).filter(
        users_read__id=user.id)
...
def send_message(sender, instance, **kwargs):
    ...
    recipient_list = [
        user.subscriber.email for user in subscribers_list if user.subscriber.email]

    subject = 'Новый пост от автора {}'.format(instance.author)
    message = 'Новый пост доступен по ссылке: {}'.format(
        host + instance.get_absolute_url())
